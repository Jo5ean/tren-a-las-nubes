  ...
  return (
    <section id="recorrido" className="border-y border-puna-200 bg-puna-100/40">
      <div className="container-content py-20 sm:py-28">
        ...
        {/* Ruta horizontal con estaciones en zig-zag (desktop). */}
        <div className="relative mt-20 hidden lg:block overflow-visible">
          {/* Tramo 1: calle/ruta hasta San Antonio (0-62%). */}
          <div aria-hidden="true" className="pointer-events-none absolute left-0 right-[38%] top-1/2 h-12 -translate-y-1/2">
            {/* Línea central punteada (línea de ruta). */}
            <div
              className="absolute inset-x-0 top-1/2 h-[2px] -translate-y-1/2"
              style={{
                backgroundImage:
                  'repeating-linear-gradient(90deg, #c0b3a1 0 12px, transparent 12px 24px)',
              }}
            />
            {/* Bordes de la ruta (líneas continuas). */}
            <div className="absolute inset-x-0 top-1 h-[1px] bg-puna-300/40" />
            <div className="absolute inset-x-0 bottom-1 h-[1px] bg-puna-300/40" />
          </div>

          {/* Tramo 2: vía/riel desde San Antonio (62-100%). */}
          <div aria-hidden="true" className="pointer-events-none absolute left-[62%] right-0 top-1/2 h-7 -translate-y-1/2">
        ...
        {/* Ruta vertical con vehículos descendentes (mobile / tablet). */}
        <div className="relative mt-12 lg:hidden">
          {/* Tramo 1: calle/ruta hasta San Antonio (0-62%). */}
          <div aria-hidden="true" className="absolute bottom-[38%] left-4 top-0 w-12 -translate-x-1/2">
            {/* Línea central punteada (línea de ruta). */}
            <div
              className="absolute inset-y-0 left-1/2 w-[2px] -translate-x-1/2"
              style={{
                backgroundImage:
                  'repeating-linear-gradient(0deg, #c0b3a1 0 12px, transparent 12px 24px)',
              }}
            />
            {/* Bordes de la ruta (líneas continuas). */}
            <div className="absolute inset-y-0 left-1 h-[1px] w-[1px] bg-puna-300/40" />
            <div className="absolute inset-y-0 right-1 h-[1px] w-[1px] bg-puna-300/40" />
          </div>

          {/* Tramo 2: vía/riel desde San Antonio (62-100%). */}
          <div aria-hidden="true" className="absolute bottom-0 left-4 top-[62%] w-7 -translate-x-1/2">
